...
void unmj_drawStringFont( char *name, uint32_t size )
{
	...
	sprintf(fontname, "-*-%s-*-r-*-*-%lu-*-*-*-*-*-*-*", name, size);
...
void createFrame()
{
	...
	for(int i=0; i < windowInfo.height; i++)
		{
			for(int j=0, x, y; j < windowInfo.width; j++)
				{
					/* Select image to display at a particular (x,y) window coordinate */
...
short detObstacles(int min, int max, int pos, int *obstacle, int *n, int obstacleLine, bool horizontal)
{
	int i, j;
